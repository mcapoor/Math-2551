...
Then,
\begin{center}
    Position: $\quad \vec{r} = r \vec{u}_r + z \hat{k}$\\
    Velocity: $\quad \vec{v} = \dot{r} \vec{u}_r + r \dot{\theta} \vec{u}_\theta + \dot{z} \hat{k}$\\
    Acceleration: $\quad \vec{a} = (\ddot{r} - r \dot{\theta}^2) \vec{u}_r + (r\ddot{\theta} + 2\dot{r} \ddot{\theta}) \vec{u}_\theta + \ddot{z} \hat{k}$\\
\end{center}
...
\emph{Kepler's Laws:}
\begin{enumerate}
    \item A planet's path is an ellupse with the sun at one focus. The eccentricity is 
